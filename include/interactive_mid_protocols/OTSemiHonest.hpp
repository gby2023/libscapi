 ...
 * Semi-Honest OT has two modes: one is on ByteArray and the second is on
 * GroupElement. The difference is in the input and output types and the way to
 * process them. In spite that, there is a common behavior for both modes which
 * this class implements.
 *
 * The pseudo code of this protocol can be found in Protocol 4.1 of pseudo codes
 * document at
...
class OTSemiHonestDDHSenderAbs : public OTSender {
 ...
 protected:
   ...
   */
  OTSemiHonestDDHSenderAbs(const shared_ptr<PrgFromOpenSSLAES>& random,
   ...
   */
  virtual shared_ptr<OTSMsg> computeTuple(OTSInput* input, GroupElement* u,
                                          GroupElement* k0,
                                          GroupElement* k1) = 0;
 ...
 private:
   ...
   */
  shared_ptr<OTRGroupElementPairMsg> waitForMessageFromReceiver(
      CommParty* channel);
   ...
   */
  shared_ptr<GroupElement> computeK0(biginteger& r,
                                     OTRGroupElementPairMsg* message);
   ...
   */
  shared_ptr<GroupElement> computeK1(biginteger& r,
                                     OTRGroupElementPairMsg* message);
...
class OTSemiHonestDDHOnGroupElementSenderMsg : public OTSMsg {
 ...
 public:
   ...
   */
  OTSemiHonestDDHOnGroupElementSenderMsg(
      const shared_ptr<GroupElementSendableData>& u,
      const shared_ptr<GroupElementSendableData>& v0,
      const shared_ptr<GroupElementSendableData>& v1)
      : u(u), v0(v0), v1(v1) {}
 ...
 * In the byteArray scenario, the sender sends GroupElement u and two binary
 * strings v0, v1.
 *
 */
class OTSemiHonestDDHOnByteArraySenderMsg : public OTSMsg {
 ...
 public:
   ...
   */
  OTSemiHonestDDHOnByteArraySenderMsg(
      const shared_ptr<GroupElementSendableData>& u, vector<byte>& v0,
      vector<byte>& v1)
      : u(u), v0(v0), v1(v1) {}
 ...
 * This class derived from OTSemiHonestDDHSenderAbs and implements the
 ...
 *
 */
class OTSemiHonestDDHOnGroupElementSender : public OTSemiHonestDDHSenderAbs,
                                            SemiHonest {
 public:
  /**
   * Constructor that sets the given dlogGroup and random.
 ...
 protected:
   ...
   * @return tuple contains (u, v0, v1) to send to the receiver.
   */
  shared_ptr<OTSMsg> computeTuple(OTSInput* input, GroupElement* u,
                                  GroupElement* k0, GroupElement* k1) override;
};
 ...
 *
 */
class OTSemiHonestDDHOnByteArraySender : public OTSemiHonestDDHSenderAbs,
                                         SemiHonest {
 public:
  /**
   ...
   * @throws SecurityLevelException if the given DlogGroup is not DDH secure.
   */
  OTSemiHonestDDHOnByteArraySender(
      const shared_ptr<PrgFromOpenSSLAES>& random =
          make_shared<PrgFromOpenSSLAES>(),
      const shared_ptr<DlogGroup>& dlog =
          make_shared<OpenSSLDlogECF2m>("K-233"),
      ...
      : OTSemiHonestDDHSenderAbs(random, dlog), kdf(kdf) {}
 ...
 * Semi-Honest OT have two modes: one is on ByteArray and the second is on
 * GroupElement. The different is in the input and output types and the way to
 * process them. In spite that, there is a common behavior for both modes which
...
class OTSemiHonestDDHReceiverAbs : public OTReceiver {
  /*
     ...
     in byte array scenario
          OR (kSigma)^(-1) = u^(-alpha)					- in
     GroupElement scenario OUTPUT  xSigma = vSigma XOR KDF(|cSigma|,kSigma)
     - in byte array scenario
          OR xSigma = vSigma * (kSigma)^(-1) 			- in GroupElement
     scenario
  */
 ...
 public:
   ...
   *		SEND (h0,h1) to S
   *		WAIT for the message (u, v0,v1) from S
   *		COMPUTE kSigma = (u)^alpha							- in byte
   *array scenario
   *			 OR (kSigma)^(-1) = u^(-alpha)					- in
   *GroupElement scenario OUTPUT  xSigma = vSigma XOR KDF(|cSigma|,kSigma)
   *- in byte array scenario
   *			 OR xSigma = vSigma * (kSigma)^(-1)" 			- in
   *GroupElement scenario
   */
 ...
 protected:
   ...
   * @throws SecurityLevelException if the given dlog is not DDH secure.
   */
  OTSemiHonestDDHReceiverAbs(const shared_ptr<PrgFromOpenSSLAES>& random,
                             const shared_ptr<DlogGroup>& dlog);

  /**
  * Runs the following lines from the protocol:
  * "COMPUTE kSigma = (u)^alpha							- in
  byte array scenario
  OR (kSigma)^(-1) = u^(-alpha)							- in
  GroupElement scenario OUTPUT  xSigma = vSigma XOR KDF(|cSigma|,kSigma)
  - in byte array scenario OR xSigma = vSigma * (kSigma)^(-1)
 ...
 private:
   ...
   */
  shared_ptr<OTRGroupElementPairMsg> computeTuple(biginteger& alpha,
                                                  bool sigma);
 ...
 * This class derived from OTSemiHonestDDHReceiverAbs and implements the
 * functionality related to the GroupElement inputs.
 *
 * The pseudo code of this protocol can be found in Protocol 4.1 of pseudo codes
 * document at
 ...
 *
 */
class OTSemiHonestDDHOnGroupElementReceiver : public OTSemiHonestDDHReceiverAbs,
                                              SemiHonest {
 public:
  /**
   * Constructor that sets the given dlogGroup and random.
   ...
   * @throws SecurityLevelException if the given DlogGroup is not DDH secure.
   */
  OTSemiHonestDDHOnGroupElementReceiver(
      const shared_ptr<PrgFromOpenSSLAES>& random =
          make_shared<PrgFromOpenSSLAES>(),
      const shared_ptr<DlogGroup>& dlog =
          make_shared<OpenSSLDlogECF2m>("K-233"))
      : OTSemiHonestDDHReceiverAbs(random, dlog) {}
 ...
 protected:
   ...
   * @param sigma input for the protocol
   * @param alpha random value sampled by the protocol
   * @param message received from the sender. must be
   *OTSOnGroupElementSemiHonestMessage
   * @return OTROutput contains xSigma
   */
  shared_ptr<OTROutput> getMsgAndComputeXSigma(CommParty* channel, bool sigma,
                                               biginteger& alpha) override;
};
 ...
 *
 */
class OTSemiHonestDDHOnByteArrayReceiver : public OTSemiHonestDDHReceiverAbs,
                                           SemiHonest {
 public:
  /**
   * Constructor that sets the given dlogGroup, kdf and random.
   ...
   * @throws SecurityLevelException if the given DlogGroup is not DDH secure.
   */
  OTSemiHonestDDHOnByteArrayReceiver(
      const shared_ptr<PrgFromOpenSSLAES>& random =
          make_shared<PrgFromOpenSSLAES>(),
      const shared_ptr<DlogGroup>& dlog =
          make_shared<OpenSSLDlogECF2m>("K-233"),
      const shared_ptr<KeyDerivationFunction>& kdf =
          make_shared<HKDF>(make_shared<OpenSSLHMAC>("SHA-256")))
      : OTSemiHonestDDHReceiverAbs(random, dlog), kdf(kdf) {}
 ...
 protected:
   ...
   * @param alpha random value sampled by the protocol
   * @param message received from the sender. must be
   *OTSOnByteArraySemiHonestMessage.
   * @return OTROutput contains xSigma
   */
  shared_ptr<OTROutput> getMsgAndComputeXSigma(CommParty* channel, bool sigma,
                                               biginteger& alpha) override;
