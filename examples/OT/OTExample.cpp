...
shared_ptr<OTSInput> getInput(DlogGroup *dlog, OTParams params)
{
	...
	if (params.protocolName == "SemiHonestOnGroupElement" || params.protocolName == "PrivacyOnlyOnGroupElement" ||
		params.protocolName == "OneSidedSimulationOnGroupElement" || params.protocolName == "FullSimulationOnGroupElement" || params.protocolName == "FullSimulationROMOnGroupElement" || params.protocolName == "UCOnGroupElement")
	{
		...
		return make_shared<OTOnGroupElementSInput>(x0, x1);
			 ...
			 params.protocolName == "OneSidedSimulationOnByteArray" || params.protocolName == "FullSimulationOnByteArray" || params.protocolName == "FullSimulationROMOnByteArray" || params.protocolName == "UCOnByteArray")
	{
		vector<unsigned char> x0(10, '0'), x1(10, '1');
		cout << "x0 = " << endl;
...
int mainOT(string side, string configPath)
{
	...
	shared_ptr<CommParty> server = (side == "1") ? make_shared<CommPartyTCPSynced>(io_service, senderParty, receiverParty) : make_shared<CommPartyTCPSynced>(io_service, receiverParty, senderParty);
