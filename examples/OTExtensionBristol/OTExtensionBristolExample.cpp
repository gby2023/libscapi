...
int mainBristol(string partyNum)
{
	...
	if (my_num == 1)
	{
		...
		vector<unsigned char> outputbytes = ((OTExtensionRandomizedSOutput *)output.get())->getR0Arr();
	...
	else
	{
		...
		vector<unsigned char> outputbytes = ((OTOnByteArrayROutput *)output.get())->getXSigma();
	...
	/*if (my_num == 0) {
		 ...
		 vector<unsigned char> x0Arr;
		 x0Arr.resize(nOTs * elementSize/8);

		 vector<unsigned char> x1Arr;
	...
	/*
		...
		if (my_num == 1) {
				...
				auto output = sender.transfer(input);
				 ...
				 vector<unsigned char> outputbytes = ((OTExtensionCorrelatedSOutput *)output.get())->getx0Arr();
