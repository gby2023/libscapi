...
void OTExtensionBristolBase::init(const string &senderAddress, int port,
                                  int my_num, bool isSemiHonest,
  ...
  pOtExt.reset(new OTExtensionWithMatrix(
      128, baseOT.length(), 1, 1, pParty.get(), baseReceiverInput,
      baseOT.sender_inputs, baseOT.receiver_outputs, ot_role, isSemiHonest));
}

void OTExtensionBristolBase::transfer(int nOTs,
...
void OTExtensionBristolBase::shrinkArray(int sourceElementSize,
                                         int targetElementSize,
                                         int numOfElements, byte *srcArr,
                                         byte *targetArr) {
  ...
  for (int i = 0; i < numOfElements; i++) {
    for (int j = 0; j < targetElementSize / 8; j++) {
      ...
      targetArr[targetElementSize * i / 8 + j] =
          srcArr[sourceElementSize * i / 8 + j];
    }
  }
}

OTExtensionBristolSender::OTExtensionBristolSender(
    int port, bool isSemiHonest, const shared_ptr<CommParty> &channel) {
  // Call the init of the base class. The host name is hard coded to localhost
  // since the sender is the  listener.
  init("localhost", port, 0, isSemiHonest, channel);
}

shared_ptr<OTBatchSOutput> OTExtensionBristolSender::transfer(
    OTBatchSInput *input) {
  if (input->getType() != OTBatchSInputTypes::OTExtensionRandomizedSInput &&
    ...
    throw invalid_argument(
        "input should be instance of OTExtensionRandomizedSInput or "
        "OTExtensionGeneralSInput or OTExtensionCorrelatedSInput.");
  } else {
    ...
    // we create a bitvector since the transfer of the bristol library demands
    // that. There is no use of it and thus we do not require that the user
    // inputs that.
    BitVector receiverInput(nOTs);

    // call the base class transfer that eventually calls the ot extension of
    // the bristol library
    ...
    if (input->getType() == OTBatchSInputTypes::OTExtensionGeneralSInput) {
      if (channel == NULL) {
        throw runtime_error(
            "In order to execute a general ot extension the channel must be "
            "given");
      }
      ...
      // using the channel member, since OT bristol only works on random case.
    } else if (input->getType() ==
               OTBatchSInputTypes::OTExtensionCorrelatedSInput) {
      if (channel == NULL) {
        throw runtime_error(
            "In order to execute a correlated ot extension the channel must be "
            "given");
      }
...
void OTExtensionBristolBase::expandOutput(int elementSize, byte *key,
                                          vector<byte> &output, int factor,
                                          const byte *counters,
                                          EVP_CIPHER_CTX *aes, byte *aesOutput,
                                          int i) const {
...
byte *OTExtensionBristolBase::createCountersArray(int factor)
    const {  // Create the array of indices to be the plaintext for the AES
...
OTExtensionBristolReceiver::OTExtensionBristolReceiver(
    const string &senderAddress, int port, bool isSemiHonest,
    const shared_ptr<CommParty> &channel) {
  init(senderAddress, port, 1, isSemiHonest, channel);
}

shared_ptr<OTBatchROutput> OTExtensionBristolReceiver::transfer(
    OTBatchRInput *input) {
  if (input->getType() != OTBatchRInputTypes::OTExtensionGeneralRInput &&
    ...
    for (size_t i = 0; i < nOTsReal; i++) {
      if (sigmaArr[i] == 1) inputBits.set_bit(i, 1);
    }
